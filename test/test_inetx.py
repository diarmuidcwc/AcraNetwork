...
class iNetXTest(unittest.TestCase):
    def test_defaultiNet(self):
        i = inetx.iNetX()
        self.assertEqual(i.packetlen, 0)
        self.assertEqual(i.inetxcontrol, inetx.iNetX.DEF_CONTROL_WORD)
        self.assertEqual(i.ptptimeseconds, 0)
        self.assertEqual(i.ptptimenanoseconds, 0)
        self.assertEqual(i.pif, 0)
        self.assertEqual(i.payload, bytes())
        self.assertEqual(i.sequence, 0)
        self.assertEqual(i.streamid, 0)
    ...
    def test_unpackiNet(self):
        expected_payload = struct.pack(
            inetx.iNetX.INETX_HEADER_FORMAT, inetx.iNetX.DEF_CONTROL_WORD, 0xDC, 2, 30, 1, 1, 0
        ) + struct.pack("H", 0x5)
