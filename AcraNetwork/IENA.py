...
class IENA(object):
    ...
    def __init__(self):
        self._key = 0  # know as ienakey
        self.size: int = 0  #: IENA packet size incl header. This is calculated automatically when packing the message
        self.timeusec: int = 0  #: Time of the first byte in the payload in us since Jan 1st of the current year
        self.keystatus: int = 0  #: Key Status. Fully described in the IENA standard
        self.status: int = 0  #: N2 Status. Fully described in the IENA standard
        self.sequence: int = 0  #: Sequence Number. Circular counter unique per key. Wraps at 16 bits.
        self.endfield: int = 0xDEAD  #: Trailer field in the IENA packet
        self.payload: bytes = bytes()  #: Payload of the IENA packet
    ...
    def setPacketTime(self, utctimestamp: int, microseconds: int = 0) -> None:
        """
...
class IENAQ(IENA):
    ...
    def __init__(self):
        ...
        self.parameters = (
            []
        )  #: The list of all QParameters in thie IENA-Q packet Each entry is of class :class:`QParameter`
        self._req_attr = IENAM.REQ_ATTR
    ...
    def unpack(self, buf: bytes):
        ...
        while len(remaining_payload) > 0:
            ...
            qparam = QParameter(
                paramid=_paramid, dataset=remaining_payload[IENAQ._FORMAT_LEN_ : IENAQ._FORMAT_LEN_ + datasetlength]
            )
...
class IENAD(IENA):
    ...
    def unpack(self, buf: bytes):
        ...
        if len(self.payload) - (num_params * len_param_bytes) != 0:
            raise ValueError(
                "There are not an integer number of D Parameters in the payload. Length DParam={} "
        ...
        for param in range(num_params):
            try:
                ...
                raise IndexError(
                    "Could not unpack the payload from offset {}. Is the N2 field correct? "
...
class IENAN(IENA):
    ...
    def unpack(self, buf: bytes) -> None:
        ...
        if len(self.payload) - (num_params * len_param_bytes) != 0:
            raise ValueError(
                "There are not an integer number of N Parameters in the payload. Length NParam={} "
                "Length IENA Payload={}".format(len_param_bytes, len(self.payload))
            )
